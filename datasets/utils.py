...
def spatial_sampling(frames, spatial_idx=-1, min_scale=256, max_scale=320, crop_size=224, random_horizontal_flip=True,
    ...
    if spatial_idx == -1:
        frames, _ = transform.random_short_side_scale_jitter(images=frames, min_size=min_scale, max_size=max_scale,
...
class ClassLabel_inference(object):

    def __call__(self, target):
        return target['label'], target['clip_label']
