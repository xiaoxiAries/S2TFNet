...
class MultigridSchedule(object):
    ...
    def init_multigrid(self, cfg):
        ...
        cfg.MULTIGRID.DEFAULT_T = cfg.DATA.NUM_ALL_FRAMES
        cfg.MULTIGRID.DEFAULT_S = cfg.DATA.CROP_SIZE
        ...
        if cfg.MULTIGRID.LONG_CYCLE:
            ...
            # Fine-tuning phase.
            cfg.SOLVER.LRS = cfg.SOLVER.LRS[:-1] + [cfg.SOLVER.LRS[-2], cfg.SOLVER.LRS[-1], ]
            cfg.SOLVER.MAX_EPOCH = self.schedule[-1][-1]

        elif cfg.MULTIGRID.SHORT_CYCLE:
            cfg.SOLVER.STEPS = [int(s * cfg.MULTIGRID.EPOCH_FACTOR) for s in cfg.SOLVER.STEPS]
            cfg.SOLVER.MAX_EPOCH = int(cfg.SOLVER.MAX_EPOCH * cfg.MULTIGRID.EPOCH_FACTOR)
        return cfg
    ...
    def update_long_cycle(self, cfg, cur_epoch):
        ...
        if base_s != cfg.DATA.CROP_SIZE or base_t != cfg.DATA.NUM_ALL_FRAMES:

            cfg.DATA.NUM_ALL_FRAMES = base_t
            cfg.DATA.CROP_SIZE = base_s
            cfg.TRAIN.BATCH_SIZE = base_b * cfg.MULTIGRID.DEFAULT_B

            bs_factor = (float(cfg.TRAIN.BATCH_SIZE / cfg.NUM_GPUS) / cfg.MULTIGRID.BN_BASE_SIZE)
            ...
            cfg.MULTIGRID.LONG_CYCLE_SAMPLING_RATE = cfg.DATA.SAMPLING_RATE * (cfg.MULTIGRID.DEFAULT_T // cfg.DATA.NUM_ALL_FRAMES)
            ...
            logger.info("\tDATA.NUM_FRAMES x LONG_CYCLE_SAMPLING_RATE: {}x{}".format(
                cfg.DATA.NUM_ALL_FRAMES, cfg.MULTIGRID.LONG_CYCLE_SAMPLING_RATE))
            logger.info("\tDATA.TRAIN_CROP_SIZE: {}".format(cfg.DATA.CROP_SIZE))
            return cfg, True
        else:
            return cfg, False
    ...
    def get_long_cycle_schedule(self, cfg):
        ...
        default_size = float(cfg.DATA.NUM_ALL_FRAMES * cfg.DATA.CROP_SIZE ** 2)
        ...
        for t_factor, s_factor in cfg.MULTIGRID.LONG_CYCLE_FACTORS:
            base_t = int(round(cfg.DATA.NUM_ALL_FRAMES * t_factor))
            base_s = int(round(cfg.DATA.CROP_SIZE * s_factor))
            if cfg.MULTIGRID.SHORT_CYCLE:
                shapes = [
                    [base_t, cfg.MULTIGRID.DEFAULT_S * cfg.MULTIGRID.SHORT_CYCLE_FACTORS[0],],
                    [base_t, cfg.MULTIGRID.DEFAULT_S * cfg.MULTIGRID.SHORT_CYCLE_FACTORS[1],],
                    [base_t, base_s],
                ]
            else:
        ...
        x = (cfg.SOLVER.MAX_EPOCH * cfg.MULTIGRID.EPOCH_FACTOR / sum(s[-1] for s in schedule))
