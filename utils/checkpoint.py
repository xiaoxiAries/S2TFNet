...
from S2TFNet.utils.c2_model_loading import get_name_convert_func
...
def load_part_of_resnet_checkpoint(path_to_checkpoint, model, data_parallel=True, optimizer=None):
    """
    ...
    checkpoint["model_state"] = {k: v for k, v in checkpoint["model_state"].items() if k in model_state_dict_3d}
    model_state_dict_3d.update(checkpoint["model_state"])
    ms.load_state_dict(model_state_dict_3d, strict=False)
    # Load the optimizer state (commonly not done when fine-tuning)
    if optimizer:
...
def load_part_of_checkpoint_for_other_dataset(path_to_checkpoint, model, data_parallel=True, optimizer=None):
    """
    Load the checkpoint from the given file.
    Returns:
        (int): the number of training epoch of the checkpoint.
    ...
    checkpoint["model_state"] = {k: v for k, v in checkpoint["model_state"].items() if (k in model_state_dict_3d and 'fc' not in k)}
    model_state_dict_3d.update(checkpoint["model_state"])
    ms.load_state_dict(model_state_dict_3d, strict=False)
    # Load the optimizer state (commonly not done when fine-tuning)
    if optimizer:
